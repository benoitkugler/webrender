...
func TestGridAutoColumnsRows(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"max-content", pr.GridAuto{pr.NewGridDims(pr.SToV("max-content"))}},
		{"fit-content(20%)", pr.GridAuto{pr.GridDims{Name: 'f', V1: pr.PercToV(20)}}},
		{"minmax(20px, 25px)", pr.GridAuto{pr.GridDims{'m', pr.FToPx(20), pr.FToPx(25)}}},
		{"minmax(min-content, max-content)", pr.GridAuto{pr.GridDims{'m', pr.SToV("min-content"), pr.SToV("max-content")}}},
		{"min-content max-content", pr.GridAuto{pr.NewGridDims(pr.SToV("min-content")), pr.NewGridDims(pr.SToV("max-content"))}},
	} {
...
func TestGridTemplateColumnsRows(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"[outer-edge] 20px [main-start] 1fr [center] 1fr max-content [main-end]", pr.GridTemplate{Names: []pr.GridSpec{
			...
			pr.NewGridDims(pr.FToPx(20)),
			pr.GridNames{"main-start"},
			pr.NewGridDims(pr.NewDim(1, pr.Fr).ToValue()),
			pr.GridNames{"center"},
			pr.NewGridDims(pr.NewDim(1, pr.Fr).ToValue()),
			nil,
			pr.GridDims{V1: pr.SToV("max-content")},
		...
		{"repeat(auto-fill, minmax(25ch, 1fr))", pr.GridTemplate{
			Names: []pr.GridSpec{
				...
				pr.GridRepeat{Repeat: pr.RepeatAutoFill, Names: []pr.GridSpec{
					nil,
					pr.GridDims{Name: 'm', V1: pr.NewDim(25, pr.Ch).ToValue(), V2: pr.NewDim(1, pr.Fr).ToValue()},
					nil,
				}},
				nil,
			},
		...
		{"[a] auto [b] minmax(min-content, 1fr) [b c d] repeat(2, [e] 40px) repeat(5, auto)", pr.GridTemplate{Names: []pr.GridSpec{
			pr.GridNames{"a"},
			pr.GridDims{V1: pr.SToV("auto")},
			pr.GridNames{"b"},
			pr.GridDims{Name: 'm', V1: pr.SToV("min-content"), V2: pr.NewDim(1, pr.Fr).ToValue()},
			pr.GridNames{"b", "c", "d"},
			pr.GridRepeat{Repeat: 2, Names: []pr.GridSpec{pr.GridNames{"e"}, pr.NewGridDims(pr.FToPx(40)), nil}},
			nil,
			pr.GridRepeat{Repeat: 5, Names: []pr.GridSpec{nil, pr.NewGridDims(pr.SToV("auto")), nil}},
			nil,
		}}},
	} {
